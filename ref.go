...
func (ref *Ref) start() {
	go func() {
		...
		if ref.owner.timeout > 0 && !ref.IsSystemRef() {
			ref.timeoutCancel = SendOnce(ref.owner.timeout, ref, timeoutMessage)
		}

		for {
			select {
			case msg := <-ref.mailbox:
				if ref.owner.timeout > 0 && !ref.IsSystemRef() && ref.timeoutCancel != nil {
					ref.timeoutCancel()
					ref.timeoutCancel = nil
				}
				...
				if d > ref.owner.alarm {
					logrus.Warnf("warning actor id %d msg %v elapsed %d", ref.id, d, msg)
					//ref.mailbox <- timeoutMessage
				}

				if ref.owner.timeout > 0 && !ref.IsSystemRef() {
					delay := ref.owner.timeout - d
					// 处理超时
					if atomic.LoadInt32(&ref.status) <= Running {
						if delay <= 0 {
							delay = ref.owner.timeout
						}
						ref.timeoutCancel = SendOnce(delay, ref, timeoutMessage)
					}
				}
...
// 一个新的actor
func newActor(ctx context.Context, cancel context.CancelFunc, owner *ActorSystem, id int64, handler Handler) *Ref {
	ref := &Ref{
		id:      id,
		owner:   owner,
		status:  Runnable,
...
func processSystemMessage(ref *Ref, msg interface{}) {
	...
	case *timeout:
		// 执行timeout时可能发生问题，但发生问题后，还是要继续发送stop
		func() {
			defer printPanicStack()
		...
		if err != nil {
			status := atomic.LoadInt32(&ref.status)
			logrus.Errorf("actor[id=%d]TimeOut Call stop fail current status: %d, err: %v", ref.id, status, err)
		}
	case *stop:
	...
	default:
		panic(fmt.Sprintf("actor[id=%d]Not Supper ActorSystem Message", ref.id))
	}
